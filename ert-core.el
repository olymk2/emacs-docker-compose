...
(ert-deftest pp-test-compose-file-from-param ()
  ...
  (should (equal (dc-core-fetch-dockerfile-from-param "-d --no-cache --filedev.yml") "dev.yml")))

(ert-deftest pp-test-compose-param-defaults-when-missing ()
  "test getting parameter from string returns default if missing"
  (should (equal (dc-core-fetch-dockerfile-from-param "-d --no-cache") "docker-compose.yml")))
...
(ert-deftest pp-test-compose-is-found ()
  ...
  (should (equal (dc-core-compose-exists-check) t)))
